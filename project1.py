...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta_final = 1 / (T*feature_matrix.shape[0]) * sum_theta
    theta_0_final = 1 / (T*feature_matrix.shape[0]) * sum_theta_0
...
def classifier_accuracy(
    ...
    train_acurc = accuracy(train_pred, train_labels)
    val_accurc = accuracy(val_pred, val_labels)
    return train_acurc, val_accurc
...
def bag_of_words(texts):
    ...
    stop_word_dic = stop_word()
    for text in texts:
        word_list = extract_words(text)
        for word in word_list:
            if word not in dictionary and word not in stop_word_dic:
...
def stop_word():
    ...
    # dic = { x.strip:"" for x in df.values}
    # return dic
